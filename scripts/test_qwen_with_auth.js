// Test script to verify Qwen model fix with authentication
...
async function testGenerateComment(token) {
    try {
...
async function main() {
    ...
    if (token) {
        // 2. Test AI comment generation
        await testGenerateComment(token);
    } else {
